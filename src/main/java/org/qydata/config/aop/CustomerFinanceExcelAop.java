    ...
    public Object queryAllCustomerExportExcel(ProceedingJoinPoint point) throws Throwable {
        ...
        if (args[0] != null && args[0].getClass() == String.class && args[0].equals("true")) {
            ...
            for ( int i = 0; i < customerFinanceList.size(); i++ ){
                CustomerFinance finance = customerFinanceList.get(i);
                Map<String, Object> mapValue = new HashMap<>();
                mapValue.put("companyName", finance.getCompanyName());
                mapValue.put("partnerName", finance.getPartnerName());
                mapValue.put("chargeWeekTotleAmount", ExportDataHander.pointsIntoRMB(finance.getLastWeekCharge()));
                mapValue.put("consumeWeekTotleAmount", ExportDataHander.pointsIntoRMB(finance.getLastWeekConsume()));
                mapValue.put("chargeMonthTotleAmount", ExportDataHander.pointsIntoRMB(finance.getLastMonthCharge()));
                mapValue.put("consumeMonthTotleAmount", ExportDataHander.pointsIntoRMB(finance.getLastMonthConsume()));
                mapValue.put("chargeTotleAmount", ExportDataHander.pointsIntoRMB(finance.getChargeTotleAmount()));
                mapValue.put("consumeTotleAmount", ExportDataHander.pointsIntoRMB(finance.getConsumeTotleAmount()));
                mapValue.put("balance", ExportDataHander.pointsIntoRMB(finance.getBalance()));
                mapValue.put("currMonthAmount", ExportDataHander.pointsIntoRMB(finance.getCurrMonthAmount()));
                mapValue.put("currDayAmount", ExportDataHander.pointsIntoRMB(finance.getCurrDayAmount()));
